...
export default function NewContractorPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(typeof body.error === "string" ? body.error : "Could not add contractor");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col gap-5">
      <h1 className="text-xl font-semibold text-fg">Add contractor</h1>
      ...
      >
        ...
        >
          {loading ? <Spinner className="h-4 w-4" /> : "Add contractor"}
        </button>
      </form>
    </div>
  );
