...
export default function PerformanceError({
  ...
  useEffect(() => {
    console.error("[performance] render error", error);
  }, [error]);

  return (
    <div className="flex flex-col items-center gap-3 rounded-xl border border-edge bg-surface p-8 text-center">
      <h2 className="text-base font-semibold text-fg">Couldn&apos;t load performance data</h2>
      <p className="max-w-md text-sm text-fg-muted">
        Something went wrong while computing the department scores. This is usually temporary — try again in a moment.
      </p>
      <button
        onClick={() => reset()}
