  ...
  return (
    <div className="flex flex-col gap-1.5 p-4">
      <div className="flex items-start justify-between gap-2">
        <div>
          <span className="font-mono text-xs text-zinc-400 dark:text-zinc-500">{step.stepCode}</span>{" "}
          <span className="font-medium text-zinc-900 dark:text-zinc-50">{step.stepName}</span>
        </div>
      ...
      {step.status === "blocked" && step.blockedReason && (
        <div className="mt-1 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700 dark:bg-red-900/30 dark:text-red-300">
...
export default async function ProjectDetailPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">{project.clientAddress}</p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            ...
            {canEditEverything && (
              <Link
                ...
                className="flex h-8 items-center justify-center rounded-lg border border-zinc-300 px-3 text-xs font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
              >
                Edit project
              </Link>
            )}
        ...
        <div className="grid grid-cols-2 gap-4 border-t border-zinc-100 pt-4 text-sm sm:grid-cols-4 dark:border-zinc-800">
          <div>
            <div className="text-zinc-400 dark:text-zinc-500">Phase</div>
            <div className="font-medium text-zinc-900 dark:text-zinc-50">{PHASE_LABELS[project.currentPhase]}</div>
          </div>
          <div>
            <div className="text-zinc-400 dark:text-zinc-500">Glass type</div>
            <div className="font-medium capitalize text-zinc-900 dark:text-zinc-50">{project.glassType}</div>
          </div>
          <div>
            <div className="text-zinc-400 dark:text-zinc-500">Final cost</div>
            <div className="font-medium text-zinc-900 dark:text-zinc-50">{formatINR(Number(project.finalCost))}</div>
          </div>
          <PaymentEditor
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Step timeline</h2>
          {canEditEverything && (
            <span className="text-xs text-zinc-400 dark:text-zinc-500">
              Editable — status, blocking, and dates
            </span>
          )}
        </div>
      ...
      <div className="flex flex-col gap-3">
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Blocker history</h2>
            ...
            {blockerHistory.map((entry, i) => (
              <div key={i} className="flex flex-col gap-1 p-4 text-sm">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <span className="font-mono text-xs text-zinc-400 dark:text-zinc-500">{entry.stepCode}</span>{" "}
                    <span className="font-medium text-zinc-900 dark:text-zinc-50">{entry.stepName}</span>
                  </div>
                  <span
                ...
                {entry.reason && <div className="text-zinc-600 dark:text-zinc-300">{entry.reason}</div>}
